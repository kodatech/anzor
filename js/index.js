...
function load_new_scan(data){
    if (data){
        ...
        htmlstr += '		<div>Qty:</div>';
        htmlstr += '		<div><input type="text" id="'+box_counter+'" value="1"></div>';
        htmlstr += '		<div>' +
...
//onkeypress=checkQty(this,\'' + stockcode + '\',\'' + box_counter + '\')"
        $('#'+box_counter).on('keyup', function () {
            //$('div').text('key up ' + ++counter);
            $("#total"+box_counter).text(box_counter);
        });
        box_counter++;
    }else{
        alert("wrong product");
